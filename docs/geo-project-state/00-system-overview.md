...
High-level architecture of the GEO Analyzer: how **monthly configuration**, the **fixed scoring engine**, the **Explain layer**, and **recommendations** fit together. For authoritative rules and **system invariants**, see **`11-system-philosophy-and-architecture-rules.md`** (especially **§9 — System invariants**).
...
6. **Recommendation layer** — Action plans and narrative guidance from **deterministic** rules, templates, and optional monthly **`guideRules`** (no Gemini on this path).
...
| Rules for contributors (includes **system invariants**) | `11-system-philosophy-and-architecture-rules.md` |

---

## Full system architecture (umbrella)

The following sections provide a **complete system-level view**: **architecture layers**, **data flow**, and **component responsibilities** at product and platform scale.
...
| **Recommendation layer** | Turns signals and opportunities into **narrative strategy** using **deterministic** templates and optional monthly **`guideRules`**. **Prioritizes** direction of travel. (Optional Gemini-based **writing examples** are a separate user-triggered API.) |
...
- Together they implement the philosophy in **`09-geo-research-policy.md`** and **`10-scoring-issue-philosophy.md`**: monthly config **extends** and **tunes**, not **replaces**, the core pipeline (see **§9 — System invariants** in **`11-system-philosophy-and-architecture-rules.md`**).
